...
- For a locally generated 360 dream, OpenCubeDiff jointly imagines six connected directions and SPAG4D SHARP-360 lifts them into one depth-aligned Gaussian field. The original direction is observed; the rest is visibly disclosed as generated.
- Detail mode restores small or compressed inputs with a conservative weak-denoise Real-ESRGAN pass before reconstruction.
...
This directly emphasizes the published judging priorities: originality through a new spatial art medium (30%), meaningful neural/GPU/browser technology (25%), and an explorable audience experience (20%), while the tested local workflow and restrained interface support execution (15%) and theme alignment (10%).
...
4. Enable Explore and make one short sideways translation to reveal parallax, then reset to the measured view.
...
- [ ] Explain that free-flight movement is unlimited while reliable reconstructed evidence is strongest near captured viewpoints.
- [ ] Do not claim factual unseen geometry or a World Labs-equivalent local world model.
